...
/**
 * Tippy.js annotation as a custom element.
 */
class AxeAnnotation extends HTMLElement {
  connectedCallback() {
    const template = document.getElementById(
      'wagtail-annotation-template',
    ).content;
    ...
    btn.className = 'wagtail-annotation__toggle';
    btn.innerText = 'Error';
    ...
    content.innerHTML = this.dataset.message;
    ...
    console.log(tippy(btn, tippyOptions));
  }
}
...
class Userbar extends HTMLElement {
  ...
  connectedCallback() {
    ...
    function isFocusOnItems() {
      return (
        ...
        !!shadowRoot.activeElement.closest('.wagtail-userbar-items')
      );
    }
  ...
  async initialiseAxe() {
    ...
    count.textContent = results.violations.length;
    this.trigger.appendChild(count);
    results.violations.forEach((violation) => {
      const annotation = document.createElement('wagtail-annotation');
      annotation.setAttribute('data-message', violation.description);
      annotation.setAttribute('data-report', JSON.stringify(violation));
...
customElements.define('wagtail-annotation', AxeAnnotation);
